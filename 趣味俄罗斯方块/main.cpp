...
void welcom()
{
	...
	int i,j = 1;
	color(14);          			//黄色边框
	for (i = 9; i <= 20; i++)   	//输出上下边框Ψ
	{
		for (j = 15; j <= 60; j++)  //输出左右边框§
		{
			gotoxy(j, i);
			if (i == 9 || i == 20) printf("Ψ");
...
int ifMove(struct Tetris *tetris)
{
 	...
 	{
  		if( //当为田字方块且除中心方块位置外，其他"ㄖ"字方块位置上无图案时，说明这个位置能够放下田字方块，可以移动到这个位置，返回值为1，即可移动
			//比如田字方块，它的中心方块是左下角的ㄖ，如果它的上，右，右上的位置为空，则这个位置就可以放一个田字方块；如果有一个位置上不为空，都放不下一个田
   		( tetris->flag==1  && ( a[tetris->x][tetris->y-1]==0   &&
    	a[tetris->x+2][tetris->y-1]==0 && a[tetris->x+2][tetris->y]==0 ) ) ||
   		//或为直线方块且除中心方块位置外，其他"ㄖ"字方块位置上无图案时，返回值为1，即可移动
...
void CleanTetris(struct Tetris *tetris)
{
	...
	for( i = tetris->x - 2;i <= tetris->x + 4; i+=2 )       //ㄖX ㄖㄖ  X为中心方块
	{
  		for(j = tetris->y-2;j <= tetris->y + 1;j++)        /* ㄖ
															  ㄖ
															  X
  															  ㄖ	*/
  		{
